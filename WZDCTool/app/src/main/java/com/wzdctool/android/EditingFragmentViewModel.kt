...
class EditingFragmentViewModel : ViewModel() {
    ...
    fun initMap(mMap: GoogleMap, mMapView: MapView) {
        ...
        for (marker in localVisualizationObj.markers) {
            ...
            if (title.type == "LC") {
                iconID = laneClosedIcons[title.value.toInt() - 1]
                draggable = true
            }
            else if (title.type == "LO") {
                iconID = laneOpenIcons[title.value.toInt() - 1]
                draggable = true
            }
...
//                    TODO("Not yet implemented")
        ...
        mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(localVisualizationObj.dataPoints[0], 15f))
    }
